...
export const useStore = defineStore('store', () => {
    ...
    const currUser = ref(storedSession ? JSON.parse(storedSession) : null)

    const capitalUser = computed(() => {
        if (!currUser.value) return ''
        const name = currUser.value.user
    ...
    return { currUser, capitalUser, targetUser, capitalTarget, users, messages, signIn, createAcc, signOut, sendMsg, sendReq, acceptReq }
})